...
- [x] **Fed daimon's audit events into the libro chain** (the P2). New `src/audit.cyr` owns the chain (genesis-seeded at `daimon_audit_init`, shared with the libro tools) and exposes `daimon_audit` / `daimon_audit_agent`. Instrumented events: `agent.spawn` / `agent.stop` (INFO), `ipc.auth.deny` (SECURITY — SO_PEERCRED UID mismatch), `http.ratelimit` (WARNING — per-IP 429), `mcp.register.reject` (SECURITY — SSRF-guard reject), `mcp.call` (INFO — external tool forward). Helpers no-op until the chain is up; security-sensitive data kept out of the entry `details` (libro's tool JSON emits severity/source/action/agent_id, not details). Integration smoke extended — an SSRF-rejected registration shows on the chain as `mcp.register.reject` via `libro_export`. 225/225 tests + 6/6 smoke checks + 5 fuzz pass.

## v1.3.0 — Current work arc

The libro-tool wiring + audit feed have both landed; no open follow-ups remain on the 1.3.0 arc.
