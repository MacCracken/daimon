...
- [ ] **Collapse `serve_async` to `sandhi_server_run_opts`** when sandhi 0.8.0 ships its multi-conn accept model — at that point daimon's epoll loop in `serve_async` (and the per-call buf alloc, and the inline smuggling-check duplication) can collapse back into a single `sandhi_server_run_opts(...)` call shared with sync. Watch sandhi CHANGELOG for the 0.8.0 milestone.
