...
- **On AGNOS** (2.4.0): daimon's agnos arms (spawn, signals, channels, its polled loop). Known kernel gaps are listed, with daimon's interim for each, in `docs/audit/2026-09-23-agnos-platform-audit.md`: agents run without limits, any process can act on any TCP connection, and the listener cannot be loopback-only. A report that goes beyond them is in scope.
